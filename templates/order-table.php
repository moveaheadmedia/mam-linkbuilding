...
global $filters;
$filters['date1'] = date('d F Y', strtotime('first day of this month'));
$filters['date2'] = date('d F Y', strtotime('today'));
if (isset($_GET['dates']) && $_GET['dates'] != '') {
    $dArray = explode(' - ', $_GET['dates']);
    $filters['date1'] = $dArray[0];
    $filters['date2'] = $dArray[1];
}
...
if (is_user_logged_in()) {
    $columns_list = array("Client Name", "Client Website", "Agency", "Anchor Text", "Anchor Text Type", "Target URL", "Niche", "Sent To Writers",
        "Article sent to the site", "Live Link Received", "Live Link", "Date Paid", "USD Price", "THB Price", "Checked", "Status",
